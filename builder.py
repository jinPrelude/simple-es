...
def build_env(config):
    if config['env_name'] in ['simple_spread', 'waterworld', 'multiwalker']:
        return PettingzooWrapper(config['env_name'], config['max_step'])
    else:
        return GymWrapper(config['env_name'], config['max_step'])
...
def build_strategy(config, env, network, gen_num, process_num, eval_ep_num, log):
    if config['name'] == 'simple_evolution':
        ...
        return ESLoop(strategy, env, network, gen_num, process_num, eval_ep_num, log)
